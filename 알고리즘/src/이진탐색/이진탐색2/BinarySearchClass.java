...
public class BinarySearchClass {
	...
	public void search() { // 이진검색
		...
		int min=0; // 최소 index 
		int max=list.size()-1; // 최고 index 
		int mid=0; // 가운데 index 
		while(min<=max) { // 최대인덱스가 최소인덱스보다 크거나 같을때까지 찾는다.
			mid = (min+max)/2; // 가운데 index 
			System.out.println("mid :"+mid);
			if(list.get(mid).getUserNum() == num) { // 가운데인덱스의 회원번호가 찾는 회원번호가 같다면 출력한다.
				System.out.println(list.get(mid).toString());
				return;
			}else if(list.get(mid).getUserNum() < num) { // 찾는 회원번호가 가운데인덱스의 회원번호보다 크다면
